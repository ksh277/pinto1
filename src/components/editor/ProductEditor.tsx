'use client'
/**
 * ProductEditor.tsx (Upgraded & Type-safe)
 ...
 * - Big anchors & transparent hit padding for easy control
...
import {
  Stage, Layer, Image as KImage, Line, Rect, Circle, Group, Transformer
} from 'react-konva'
...
import { AnyShape, ShapeType } from '@/types/editor'
...
export default function ProductEditor() {
  ...
  useEffect(() => { if (imgEl) fitToBoard() }, [imgEl, s.fitToken, W, H])
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === ' ') { e.preventDefault(); setPanning(true); stage.draggable(true) }
      ...
      if (e.key === '=' || e.key === '+') setViewScale(v => clamp(+(v + 0.5).toFixed(2), 0.5, 6))
      if (e.key === '-') setViewScale(v => clamp(+(v - 0.5).toFixed(2), 0.5, 6))
      if (e.key === 'Escape') { setSelectedShapeId(null); setTool('select') }
    }
    const onKeyUp = (e: KeyboardEvent) => {
      if (e.key === ' ') { setPanning(false); stage.draggable(false) }
      if (e.key === 'Shift') setShiftDown(false)
    }
    const onBlur = () => { setPanning(false); stage.draggable(false) }
  ...
  const [holesPreview, setHolesPreview] = useState<{ cx: number; cy: number; r: number }[]>([])
  ...
  useEffect(() => {
    (async () => {
      ...
      const baseMask = imageNodeToMaskCanvas(imgEl, W, H, {
        x: s.transform.x, y: s.transform.y,
        scaleX: imgScale.x, scaleY: imgScale.y, rotation: s.transform.rotation
      })

      const drawShapes = s.shapes.map(sh => {
        if (sh.type === 'rect')
          return { type: 'rect' as const, x: sh.x, y: sh.y, width: (sh as any).width, height: (sh as any).height, rotation: sh.rotation }
        return { type: 'circle' as const, x: sh.x, y: sh.y, radius: (sh as any).radius, rotation: sh.rotation }
      })
      ...
      useEditorStore.getState().setPaths(
    ...
    imgEl,
    s.transform.x, s.transform.y, s.transform.rotation,
    imgScale.x, imgScale.y,
    s.hole.mode, s.hole.count, s.hole.side, s.hole.diaMM, s.hole.earExtraMM, s.hole.manual,
    s.cutOffsetMM, s.whiteShrinkMM, s.dpi, W, H, s.shapes,
  ])
    ...
    if (isEmpty) {
      ...
      if (tool === 'sample') {
        ...
        if (pos) {
          ...
          const canvas: HTMLCanvasElement = stage.toCanvas({ pixelRatio: 1 }) as any
  ...
  const onMouseMove = () => {
    ...
    if (!stage) return
    const pos = stage.getPointerPosition()
    if (!pos) { setCursorPos(null); return }
    const realX = (pos.x - stage.x()) / viewScale
    const realY = (pos.y - stage.y()) / viewScale
  ...
  const exportPdf = () => {
    ...
    doc.addPage([wMm, hMm], 'portrait')
    if (s.cutlinePath?.path?.length) drawPolyline(doc, s.cutlinePath.path, s.dpi, false)
  ...
  const guidesLayer = showGuides ? (
    <>
      <Line points={[W / 2, 0, W / 2, H]} stroke="#60a5fa" dash={[6, 6]} opacity={0.5} />
      <Line points={[0, H / 2, W, H / 2]} stroke="#60a5fa" dash={[6, 6]} opacity={0.5} />
    </>
  ) : null
  const safeInset = mm2px(2, s.dpi)
    ...
    <div className="flex h-full flex-col gap-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm text-neutral-500">
          드래그=이동 · 모서리=크기/회전 · 휠/버튼=줌 · <b>Space=Pan</b> · Shift=15° · Delete/Arrow=편집 · Alt+클릭=샘플
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="join">
            <button className="btn btn-sm join-item" onClick={() => setViewScale(v => clamp(+(v - 0.5).toFixed(2), 0.5, 6))}>-</button>
            <span className="btn btn-sm join-item no-animation">{Math.round(viewScale * 100)}%</span>
            <button className="btn btn-sm join-item" onClick={() => setViewScale(v => clamp(+(v + 0.5).toFixed(2), 0.5, 6))}>+</button>
            <button className="btn btn-sm join-item" onClick={() => setViewScale(1)}>100%</button>
          ...
          {/* Tools */}
          <div className="join">
            <button className={`btn btn-sm join-item ${tool==='select'?'btn-primary':'btn-ghost'}`} onClick={()=>setTool('select')}>선택</button>
            <button className={`btn btn-sm join-item ${tool==='hole'?'btn-primary':'btn-ghost'}`} onClick={()=>setTool('hole')}>구멍(수동)</button>
            <button className={`btn btn-sm join-item ${tool==='sample'?'btn-primary':'btn-ghost'}`} onClick={()=>setTool('sample')}>샘플</button>
          </div>
          <button className="btn btn-sm" onClick={() => s.clearManualHoles()} disabled={s.hole.manual.length === 0}>구멍 초기화</button>

          <div className="divider divider-horizontal" />

          <label className="label cursor-pointer gap-1 text-xs">
            <input type="checkbox" checked={snapGrid} onChange={e => setSnapGrid(e.target.checked)} /> 그리드 스냅(0.5mm)
          </label>
          <label className="label cursor-pointer gap-1 text-xs"><input type="checkbox" checked={showGrid} onChange={e => setShowGrid(e.target.checked)} /> 그리드</label>
          <label className="label cursor-pointer gap-1 text-xs"><input type="checkbox" checked={showGuides} onChange={e => setShowGuides(e.target.checked)} /> 가이드</label>
          <label className="label cursor-pointer gap-1 text-xs"><input type="checkbox" checked={showSafe} onChange={e => setShowSafe(e.target.checked)} /> 안전여백</label>
          <label className="label cursor-pointer gap-1 text-xs"><input type="checkbox" checked={showRuler} onChange={e => setShowRuler(e.target.checked)} /> 룰러</label>

          <div className="divider divider-horizontal" />
          <button className="btn btn-sm" onClick={() => addShape('rect')}>사각형</button>
          <button className="btn btn-sm" onClick={() => addShape('circle')}>원</button>
      ...
      <div className="relative h-[calc(100vh-220px)] w-full overflow-hidden rounded-xl border bg-white shadow">
        {showRuler && <Rulers width={W} height={H} dpi={s.dpi} scale={viewScale} offset={stagePos} />}
        ...
        <Stage
          ...
          y={stagePos.y}
          draggable={panning}
          onDragMove={(e: any) => { if (e.target === e.target.getStage()) setStagePos({ x: e.target.x(), y: e.target.y() }) }}
        ...
        >
          ...
          <Layer listening={false} hitGraphEnabled={false} perfectDrawEnabled={false}>
            ...
            {gridLayer}
            {guidesLayer}
            {showSafe ? <Rect x={mm2px(2,s.dpi)} y={mm2px(2,s.dpi)} width={W-mm2px(4,s.dpi)} height={H-mm2px(4,s.dpi)} stroke="#22c55e" dash={[4,4]} opacity={0.6} /> : null}
          </Layer>
          ...
          <Layer>
            {imgEl && (
              <>
                <KImage
                  ...
                  onDragMove={(e) => useEditorStore.getState().setTransform({ x: e.target.x(), y: e.target.y() })}
            ...
            {s.shapes.map((sh) => (
              <ShapeItem
                ...
                onChange={(patch) => useEditorStore.getState().updateShape(sh.id, patch)}
            ...
            {/* outlines */}
            {s.whitePath?.path?.length ? <Line points={toPoints(s.whitePath.path)} closed stroke="#16a34a" opacity={0.9} /> : null}
            {s.cutlinePath?.path?.length ? <Line points={toPoints(s.cutlinePath.path)} closed stroke="#ff2e7e" opacity={0.95} /> : null}

            {/* manual holes preview */}
            {holesPreview.map((h, i) => (
              <Circle
                ...
                onDragMove={(e) => {
                  ...
                  if (pts[i]) { pts[i] = { cx: e.target.x(), cy: e.target.y() }; useEditorStore.getState().setManualHoles(pts) }
                }}
              />
            ))}
          </Layer>
        ...
        <div className="pointer-events-none absolute bottom-0 left-0 right-0 flex justify-between gap-4 bg-white/70 px-3 py-1 text-xs text-neutral-600">
          <div>
            {cursorPos
              ? `마우스: ${px2mm(cursorPos.x, s.dpi).toFixed(2)}mm, ${px2mm(cursorPos.y, s.dpi).toFixed(2)}mm`
              : '마우스: -'}
          </div>
          <div>
            {selectedShapeId
              ? `선택: ${selectedShapeId}`
              : imgEl
                ? `이미지: ${Math.round(imgEl.width * imgScale.x)}×${Math.round(imgEl.height * imgScale.y)} px`
                : '선택 없음'}
          </div>
        </div>
      </div>
...
/* ------------------------- shape renderer -------------------------- */
function ShapeItem({
  shape, selected, onSelect, onChange, _onDelete, trRef, snapGrid, dpi
}: {
  shape: AnyShape
  selected: boolean
  onSelect: () => void
  onChange: (patch: Partial<AnyShape>) => void
  _onDelete: () => void // 이름만 바꿔 unused 경고 방지
  trRef: any
  snapGrid: boolean
  dpi: number
}) {
  ...
  const getStageScale = () => groupRef.current?.getStage?.()?.scaleX?.() ?? 1
  const hitPad = 10 / getStageScale()
  const borderColor = selected ? '#0284c7' : (hover ? '#38bdf8' : '#0ea5e9')
  const fillAlpha = shape.fillAlpha ?? 0.2
  ...
  return (
    <Group
      ...
      onTap={onSelect}
      onDblClick={onSelect}
      onMouseEnter={() => { setHover(true); setStageCursor(groupRef.current, 'move') }}
      onMouseLeave={() => { setHover(false); setStageCursor(groupRef.current, 'default') }}
    ...
    >
      ...
      {shape.type === 'rect' ? (
        <Rect x={-hitPad} y={-hitPad} width={(shape as any).width + hitPad * 2} height={(shape as any).height + hitPad * 2} opacity={0.01} fill="rgba(0,0,0,0.01)" />
      ) : (
        <Circle x={0} y={0} radius={(shape as any).radius + hitPad} opacity={0.01} fill="rgba(0,0,0,0.01)" />
      )}
...
/* --------------------------- rulers ---------------------------- */
function Rulers({
  width, height, dpi, scale, offset,
}: { width: number; height: number; dpi: number; scale: number; offset: { x: number; y: number } }) {
  ...
  for (let mm = 0; mm <= mmW; mm += 1) {
    ...
    top.push(<div key={`t${mm}`} style={{ position: 'absolute', left: x, top: 0, width: 1, height: isMajor ? 8 : 4, background: isMajor ? majorColor : minorColor }} />)
    if (isMajor) top.push(<div key={`tn${mm}`} style={{ position: 'absolute', left: x + 2, top: 8, fontSize: 10, color: majorColor }}>{mm}</div>)
  }
  const left: JSX.Element[] = []
  for (let mm = 0; mm <= mmH; mm += 1) {
    const y = Math.round(mm2px(mm, dpi) * scale + offset.y)
    const isMajor = mm % 5 === 0
    left.push(<div key={`l${mm}`} style={{ position: 'absolute', top: y, left: 0, width: isMajor ? 8 : 4, height: 1, background: isMajor ? majorColor : minorColor }} />)
    if (isMajor) left.push(<div key={`ln${mm}`} style={{ position: 'absolute', top: y + 2, left: 10, fontSize: 10, color: majorColor }}>{mm}</div>)
  }

  return (
    <>
      <div style={{ position: 'absolute', left: 0, right: 0, height: 20, background: '#f8fafc', borderBottom: '1px solid #e2e8f0', zIndex: 5 }}>{top}</div>
      <div style={{ position: 'absolute', top: 0, bottom: 0, width: 36, background: '#f8fafc', borderRight: '1px solid #e2e8f0', zIndex: 5 }}>{left}</div>
    </>
  )
}
...
function buildGrid(W: number, H: number, dpi: number) {
  ...
  for (let mm = 0; mm <= mmW; mm += 1) {
    ...
    const major = mm % 5 === 0
    elems.push(<Line key={`vx-${mm}`} points={[x, 0, x, H]} stroke={major ? '#d4d4d8' : '#eee'} strokeWidth={major ? 1 : 0.5} />)
  }
  for (let mm = 0; mm <= mmH; mm += 1) {
    const y = mm2px(mm, dpi)
    const major = mm % 5 === 0
    elems.push(<Line key={`hz-${mm}`} points={[0, y, W, y]} stroke={major ? '#d4d4d8' : '#eee'} strokeWidth={major ? 1 : 0.5} />)
  }
  return <>{elems}</>
}
...
function drawPolyline(doc: jsPDF, pts: { x: number; y: number }[], dpi: number, fill: boolean) {
